...
def get_queue_state(controlLanes,traci):
...
def get_head_waiting_time(controlLanes,traci):
...
def get_laneID(detectorIDs,traci):
    # 获取8条incoming车道的ID
    controlLanes = []
    for detector in detectorIDs:
...
def get_avarage_waiting_time(controlLanes,traci):
